    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            let menu = MenuBuilder::new(app)
                // ── App Menu ──
                .item(&SubmenuBuilder::new(app, "llm-wiki")
                    .item(&PredefinedMenuItem::about(app, Some("About llm-wiki"), None)?)
                    .separator()
                ...
                .item(&SubmenuBuilder::new(app, "File")
                    .item(&MenuItemBuilder::with_id("open_workspace", "Open Workspace...")
                        .accelerator("CmdOrCtrl+O")
                        .build(app)?)
                    .item(&MenuItemBuilder::with_id("import_files", "Import Files...")
                        .accelerator("CmdOrCtrl+I")
                        .build(app)?)
                    .separator()
                    .item(&MenuItemBuilder::with_id("compile_all", "Compile All")
                ...
                .item(&SubmenuBuilder::new(app, "View")
                    .item(&MenuItemBuilder::with_id("toggle_files", "Show Files Panel")
                        .accelerator("CmdOrCtrl+1")
                        .build(app)?)
                    .item(&MenuItemBuilder::with_id("toggle_graph", "Show Graph Panel")
                        .accelerator("CmdOrCtrl+2")
                        .build(app)?)
                    .item(&MenuItemBuilder::with_id("toggle_tables", "Show Tables Panel")
                        .accelerator("CmdOrCtrl+3")
                        .build(app)?)
                    .separator()
                    .item(&MenuItemBuilder::with_id("toggle_chat", "Show Chat Panel")
                ...
                .item(&SubmenuBuilder::new(app, "Help")
                    .item(&MenuItemBuilder::with_id("lint", "Run Wiki Health Check")
                        .build(app)?)
                    .build()?)
                .build()?;
            app.set_menu(menu)?;
            ...
            app.on_menu_event(move |app, event| {
                ...
                match id {
                    ...
                    "open_workspace" | "import_files" | "compile_all"
                    | "toggle_files" | "toggle_graph" | "toggle_tables" | "toggle_chat"
                    | "reload" | "lint" => {
